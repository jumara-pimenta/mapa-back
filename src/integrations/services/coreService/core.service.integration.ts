...
export class CoreServiceIntegration implements ICoreServiceIntegration {
  async getAllUsers(): Promise<GetAllUsersReponse[]> {
    try {
      const { data }: AxiosResponse<GetAllUsersReponse[]> = await coreApi().get(
        '/users/all',
      );
      ('/api/kanbans');
      return data;
  ...
  async logout(payload: LogoutRequest): Promise<any> {
    try {
      const { data }: AxiosResponse<any> = await coreApi().post(
        '/api/logout',
        payload,
      );
      return data;
  ...
  async verifyToken(token: string): Promise<VerifyTokenResponse> {
    try {
      const { data }: AxiosResponse<VerifyTokenResponse> = await coreApi().post(
        '/auth/verify',
        { token },
      );
      return data;
