...
export class PinRepository extends Pageable<Pin> implements IPinRepository {
  ...
  // async findAll(page: Page, filters: FiltersPinDTO): Promise<PageResponse<Pin>> {

  //   const condition = generateQueryByFiltersForPin(filters);

  //   const items = condition ? await this.repository.pin.findMany({
  //     ...this.buildPage(page),
  //     where: condition
  ...
  //   const total = condition ? await this.repository.pin.findMany({
  //     where: {
  //       ...condition
  //     }
