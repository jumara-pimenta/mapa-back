...
@Injectable()
export class EmployeeRepository extends Pageable<Employee> implements IEmployeeRepository {
  constructor(
  ...
  async findAll(page: Page, filters: FiltersEmployeeDTO): Promise<PageResponse<Employee>> {

    const condition = generateQueryByFiltersForEmployee(filters);

    const items = condition ? await this.repository.employee.findMany({
      ...this.buildPage(page),
      where: condition
    ...
    const total = condition ? await this.repository.employee.findMany({
      where: {
        ...condition
      }
