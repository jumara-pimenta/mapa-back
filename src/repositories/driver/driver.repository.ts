...
@Injectable()
export class DriverRepository extends Pageable<Driver> implements IDriverRepository {
  constructor(
  ...
  async findAll(page: Page, filters: FiltersDriverDTO): Promise<PageResponse<Driver>> {

    const condition = generateQueryByFiltersForDriver(filters);

    const items = condition ? await this.repository.driver.findMany({
      ...this.buildPage(page),
      where: condition
    ...
    const total = condition ? await this.repository.driver.findMany({
      where: {
        ...condition
      }
