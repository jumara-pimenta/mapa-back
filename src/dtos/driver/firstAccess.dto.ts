...
export class FirstAccessDriverDTO {
  ...
  @Length(7, 100, {
    message: '[password] O campo password deve conter mais de 7 caracteres.',
  })
  @Matches(passwordRgx, {
    message:
      '[password] Este campo deve conter pelo menos um caractere especial e um caractere maiúsculo.',
  })
  @Transform(({ value }: TransformFnParams) => value?.trim())
  password: string;

  @IsString({
    message: '[confirmPassword] O campo confirmPassword deve ser alfanumérico.',
  })
  @IsNotEmpty({
    message: '[confirmPassword] O campo confirmPassword deve ser preenchido.',
  })
  @Matches(passwordRgx, {
    message:
      '[confirmPassword] Este campo deve conter pelo menos um caractere especial e um caractere maiúsculo.',
  })
  @Transform(({ value }: TransformFnParams) => value?.trim())
