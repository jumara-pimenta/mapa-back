...
export class DriverController {
  ...
  @Put('/:id')
  @HttpCode(HttpStatus.OK)
  async update(@Param('id') id: string, @Body() data: UpdateDriverDTO): Promise<Driver> {
    return await this.driverService.update(id, data);
  }

  @Get()
  @HttpCode(HttpStatus.OK)
  async getAll(@Query() page: Page, @Query() filters: FiltersDriverDTO): Promise<PageResponse<MappedDriverDTO>> {
